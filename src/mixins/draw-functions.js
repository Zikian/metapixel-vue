...
export default {
    ...
    computed:{
        ...
        docWidth() { return this.$store.getters.document.width },
        docHeight() { return this.$store.getters.document.height },
    ...
    methods:{
        drawPixel(color, x, y, w = this.brushSize, h  = this.brushSize){
            // Bounds in which a pixel can be drawn
            let boundX = this.selection.x; // 0 if selection does not exist
            let boundY = this.selection.y 
            let boundW = this.selection.w || this.docWidth
            let boundH = this.selection.h || this.docHeight

            // Clamp the brush to the drawable bounds
            let nX = Math.max(boundX, x);
            let nY = Math.max(boundY, y);
            let nW = Math.min(boundX + boundW, x + w) - nX;
            let nH = Math.min(boundX + boundH, y + h) - nY;
            
            //Brush is outside of drawing bounds
            if(nW <= 0 || nH <= 0) return
            ...
            // The rectangle of tiles which the brush covers
            let tileSpanX = Math.floor(nX / this.tileWidth);
            let tileSpanY = Math.floor(nY / this.tileHeight); 
            let tileSpanX2 = Math.floor((nX + nW - 1) / this.tileWidth)
            let tileSpanY2 = Math.floor((nY + nH - 1) / this.tileHeight)

            // Get the affected tile indices
            var tileIndices = []
            for (var i = tileSpanX; i < tileSpanX2; i++) {
                for(var j = tileSpanY; j < tileSpanY2; j++){
            ...
            tileIndices.forEach((index, i) => {
                ...
                var relativeX = nX - this.hoveredTilePos[0] * this.tileWidth;
                var relativeY = nY - this.hoveredTilePos[1] * this.tileHeight;
            
                //Get intersection of tile and brush
                var clipX = Math.max(0, relativeX);
                var clipY = Math.max(0, relativeY);
                var clipW = Math.min(relativeX + nW, this.tileWidth) - clipX;
                var clipH = Math.min(relativeY + nH, this.tileHeight) - clipY;

                positions.forEach(position => {
                    var absX = clipX + position[0] * this.tileWidth;
                    var absY = clipY + position[1] * this.tileHeight;
        ...
        paintTile(x, y, tile){
            // Prevent adding position that already exists
            if (this.currentTilemap[x][y] == tile.id) return
        ...
        drawImage(imageData, x, y) {
            this.currentLayer.canvas.drawImage(imageData, x, y)
        }
    }
}
